...
public class HistoryFragment extends Fragment implements DragLayout.DragCallback {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                if (item.getItemId() == R.id.menu_clear_history) {
                    ...
                    AlertDialogFragment.showMessageDialog(calculator, "" /* title */,
                            (isAdded() ? getString(R.string.dialog_clear) : null),
                            (isAdded() ? getString(R.string.menu_clear_history) : null),
                            CLEAR_DIALOG_TAG);
                    return true;
                }
    ...
    public void onStart() {
        ...
        final Calculator activity = (Calculator) getActivity();
        activity.setFragmentKeyEventListener(mFragmentKeyEventListener);
    ...
    public void onDestroy() {
        super.onDestroy();
        ((Calculator) getActivity()).setFragmentKeyEventListener(null);
        if (mDragLayout != null) {
    ...
    /**
     *  Bug 1186628, Define interface to handle events.
     */
    public interface FragmentKeyEventListener {
        boolean onFragmentKeyEvent(KeyEvent event);
    }

    private final FragmentKeyEventListener mFragmentKeyEventListener = new FragmentKeyEventListener() {
        @Override
