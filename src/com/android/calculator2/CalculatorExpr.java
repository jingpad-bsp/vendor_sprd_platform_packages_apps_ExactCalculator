...
class CalculatorExpr {
    ...
    boolean add(int id) {
        ...
        if (isConstPiece) {
            ...
            if (s == 0) {
                ...
                s++;
                // UNISOC added for bug 1056517, When entering a decimal point, complete 0 in the expression.
                if (id == R.id.dec_point ) {
                    ((Constant)mExpr.get(0)).add(R.id.digit_0);
                }
            } else {
                ...
                if(!(last instanceof Constant)) {
                    ...
                    // UNISOC added for bug 1056517, When entering a decimal point, complete 0 in the expression.
                    if (id == R.id.dec_point ) {
                        ((Constant)mExpr.get(s-1)).add(R.id.digit_0);
                    }
                }
            }
            return ((Constant)(mExpr.get(s-1))).add(id);
    ...
    private EvalRet evalUnary(int i, EvalContext ec) throws SyntaxException {
        ...
        case R.id.fun_tan:
            ...
            //UNISOC added for bug 1226845, modify prompt for infinite result.
            UnifiedReal argdiv = null ;
            try {
                argdiv = arg.sin().divide(arg.cos());
            } catch (UnifiedReal.ZeroDivisionException e) {
                throw new ArithmeticException("infinite result");
            }
            return new EvalRet(argVal.pos, argdiv);
        case R.id.fun_ln:
            argVal = evalExpr(i+1, ec);
            if (isOperator(argVal.pos, R.id.rparen, ec)) {
                argVal.pos++;
    ...
    private void addReferencedExprs(ArrayList<Long> list, ExprResolver er) {
        //Modify for bug1519124, throw ConcurrentModificationException when monkey test.
        try {
            ArrayList<Long> result = new ArrayList<Long>(list);
            for (Token t : mExpr) {
                if (t instanceof PreEval) {
                    Long index = ((PreEval) t).mIndex;
                    if (er.getResult(index) == null && !result.contains(index)) {
                        result.add(index);
                    }
                }
            }
            ...
            Log.e("CalculatorExpr", "ConcurrentModificationException");
        }
    }
