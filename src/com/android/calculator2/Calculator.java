...
import com.android.calculator2.HistoryFragment.FragmentKeyEventListener;
...
public class Calculator extends Activity
    ...
    private static final int CLIP_DATA_LENGTH_MAX = 100;
    ...
    private final OnFormulaContextMenuClickListener mOnFormulaContextMenuClickListener =
        ...
        public boolean onPaste(ClipData clip) {
            ...
            if (uri != null && mEvaluator.isLastSaved(uri)) {
                ...
                //SPRD added for bug 1000246, return false directly if the string is too long
                String tempString = item.coerceToText(Calculator.this).toString();
                if (tempString.length() > CLIP_DATA_LENGTH_MAX){
                    Toast.makeText(Calculator.this, R.string.clip_remind_message, Toast.LENGTH_SHORT).show();
                    return false;
                }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // SPRD added for bug 1005388, avoid crash when animate state is saved by monkey test.
        CalculatorState restoreSavedState = CalculatorState.INPUT;
        if (savedInstanceState != null) {
            final CalculatorState savedState = CalculatorState.values()[savedInstanceState.getInt(KEY_DISPLAY_STATE,
                    CalculatorState.INPUT.ordinal())];
            restoreSavedState = savedState;
            if (savedState !=  CalculatorState.ANIMATE) {
                setState(savedState);
                restoreInstanceState(savedInstanceState);
            }
        }
        if (savedInstanceState == null || restoreSavedState == CalculatorState.ANIMATE) {
            mCurrentState = CalculatorState.INPUT;
            mEvaluator.clearMain();
            showAndMaybeHideToolbar();
    ...
    private void setState(CalculatorState state) {
        if (mCurrentState != state) {
            ...
            if (mCurrentState == CalculatorState.ERROR) {
                ...
                // Disable error update statusbar
//                getWindow().setStatusBarColor(errorColor);
                // SPRD added for bug 1000192, avoid no result when performing an error calculation for the first time.
                mResultText.setShouldEvaluateResult(CalculatorResult.SHOULD_NOT_EVALUATE, null);
    ...
    private void removeHistoryFragment() {
        ...
        if (manager != null && !manager.isDestroyed()) {
            // unisoc: added for bug 992995, do not popBackStack if state has saved
    ...
    public void evaluateInstantIfNecessary() {
        if (mCurrentState == CalculatorState.INPUT
            ...
            Log.d(TAG, "evaluateInstantIfNecessary(): mCalculatorFormula = " + mFormulaText.getText());
    ...
    private void reveal(View sourceView, int colorRes, AnimatorListener listener) {
        // UNISOC added for Bug 1098320/1085348/1093769, avoid Calculator NullPointerException
        if (sourceView == null) {
            return;
        }
    ...
    private void showHistoryFragment() {
        ...
        manager.beginTransaction()
                ...
                // SPRD added for bug 1000493, avoid crash by allowing state loss.
    ...
    private void displayFraction() {
        ...
        // UNISOC added for bug 1184095, avoid Calculator crash
        if (result != null) {
    ...
    /**
     * Bug 1186628, Pass events to HistoryFragment.
     */
    @Override
    public boolean dispatchKeyEvent(KeyEvent event) {
        if ((!ActivityManager.isUserAMonkey()) && (fragmentKeyEventListener != null)
    ...
    public void setFragmentKeyEventListener(FragmentKeyEventListener listener) {
        this.fragmentKeyEventListener = listener;
    }
}
